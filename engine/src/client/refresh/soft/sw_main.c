...
#if !defined(YQ2_KAIOS_UNIFIED_RENDERERS) || !defined(YQ2_KAIOS_UNIFIED_HAS_GL1)
/* Always-off stub matching gl1_main.c's real kaios_cube_test_active/
 * RI_KaiosCubeTestFrame() -- frame.c (shared by all renderers)
 * references these unconditionally, so any build without gl1's real
 * implementation linked in needs them defined too, just inert. See
 * frame.c's extern declaration comment and gl1_main.c for the real
 * (RENDERER=gl1, or RENDERER=unified built with INCLUDE_GL1=1)
 * implementation.
 *
 * When gl1 IS linked in (RENDERER=gl1 standalone, or
 * RENDERER=unified INCLUDE_GL1=1), it provides the real
 * implementation and nothing else in sw_main.c references either
 * name -- don't define a second, competing copy in that case. */
